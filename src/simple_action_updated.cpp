...
bool go_to_targer(double robot_x, double robot_y, double goal_x, double goal_y)
{

  dist = sqrt(pow((goal_x-robot_x), 2) + pow((goal_y-robot_y), 2));
  ...
  ROS_INFO("goal(x2,y2) = (%f,%f), current_pos(x1,y1) = (%f,%f)",goal_x,goal_y,robot_x,robot_y);
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  if(counter_ < 2)
  { 
    if(go_to_targer(robot_curr_pos_x_,robot_curr_pos_y_,target_list_[counter_][0],target_list_[counter_][1]))
    {
          ...
          if(hint_collector_client.call(srv_hint_collector))
          {
            ...
            if(srv_hint_collector.response.hintLoaded)
            {
              ...
              if(srv_hint_collector.response.completeHypo)
              {
                ...
                if(go_to_targer(robot_curr_pos_x_,robot_curr_pos_y_,0.0,-1.0))
                {
